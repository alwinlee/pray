<?php
    ...
    if ($groupkey=="*"){
        $table_title=$currY."祈願法會 義工名冊(住宿用-總表)";
    }
    ...
    // set column title
    $xlstitle=array("序號","條碼編號",$sweepday."前行打掃","搭遊覽車","自行往返","義工類別","組別","大組","小組",
                    "區別","教室別","母班班別","姓名","手機或易聯絡電話","性別","男","女",
                    "年齡","身體特殊狀況","住宿特殊希求","參與日期","01／29初二",
                    "初二住宿","01／30初三","初三住宿","01／31初四","初四住宿",
                    "02／01初五","交通需求","搭車","去程","回程",
                    "已交車資","自行往返","通知單發放給","研討母班","各組組長","備註","重覆報名","住宿房號");

    $xlstitleW=array(6,15,5,5,16,15,
                     15,8,12,16,16,
                     16,4,4,8,12,
                     12,4,4,4,4,
                     4,4,4,6,6,
    ...
    middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    ...
    $itemM=$col[$currC].$currR.":".$col[$currC+1].($currR+1);$objWorkSheet->mergeCells($itemM);
    middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    $item=$col[$currC].($currR+2);middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    $item=$col[$currC+1].($currR+2);middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    ...
    middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    $item=$col[$currC].($currR+2);middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    $currC++;$item=$col[$currC].($currR+2);middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    ...
    $item=$col[$currC].($currR+1);middleAlignment($objWorkSheet,$item,$xlstitle[$titleid++]);
    ...
    while($row=mysql_fetch_array($result,MYSQL_ASSOC))//MYSQL_NUM))//MYSQL_ASSOC))
    {
        ...
        $objWorkSheet->setCellValue($col[$c].$currR,$idx)
                     ->setCellValue($col[++$c].$currR," ".$row["barcode"])
                     //-> setCellValueExplicit($col[++$c].$currR,$row["barcode"],PHPExcel_Cell_DataType::TYPE_STRING)
